...
| **Verify** | live instance demo — real LLM-driven handshake + 真实 agent loop timeline + 5-dimension end-to-end exercise | ✅ **真实驱动** | `84cc0680` (V315-OBSTACK-DEMO) + **`09c82d35` (6h 改造)**。v3.15.5 版本能在**什么都没证明**的情况下 exit 0:per-run registry 空 → L4 handshake `not_found`(但 session-create 仍返 200,失败不可见);LLM 步 30s 超时短于一次 reasoning turn 且失败不致命;5 个事件手工 `/v1/events/ingest` 伪造 timeline;Observe 检查 grep 已不存在的日志文本、且从不失败。6h 全部修复:skill 自动 seed(失败即 abort)、`LLM_TIMEOUT` 默认 300s 且失败致命、**删除手工 ingest**、Observe 改为解析 OTel JSON 批次并在 series 缺失时 exit 1。**双向验证**:真跑 exit 0(560 chunk / 真实 `memory_search` tool call / 16-event timeline 含 PRE_TOOL_USE+POST_TOOL_USE+STOP / 4/4 required series);负控 exit 1(喂 6g 前的单空批次日志,准确列出 4 条缺失 series)|
