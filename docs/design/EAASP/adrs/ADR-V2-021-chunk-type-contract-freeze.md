...
enum ChunkType {
  ...
  CHUNK_TYPE_WORKFLOW_CONTINUATION = 7;   // D87 observability：workflow 继续执行事件，content = "attempt N/M"
}
```
...
| `CHUNK_TYPE_WORKFLOW_CONTINUATION` | `"workflow_continuation"` |
...
```python
ALLOWED = {"text_delta", "thinking", "tool_start", "tool_result", "done", "error", "workflow_continuation"}
...
## Implementation Record (2026-04-20)

**Branch:** `worktree-phase3.5-chunk-type`
**Commits:** 15 (5cc0e4a..0bf1cf8)

### Changes landed

| Commit | Scope |
|--------|-------|
| 5cc0e4a | proto: `ChunkType` enum (8 variants) + 4 Python stub regens + 2 new `build_proto.py` |
| e9472e4 | grid-runtime: `chunk_type_to_proto()` mapper at service.rs boundary |
| 235c626 | claw-code: 4 emission sites → `ChunkType::* as i32` |
| bbd0421 | goose: 4 emission sites (+ GOOSE_BIN-gated tests) |
| b3d1e4b | claude-code: `_CHUNK_TYPE_MAP` in mapper.py + domain `ChunkEvent.chunk_type: str` preserved |
| e7cc61c | nanobot: 5 emission sites via `common_pb2.CHUNK_TYPE_*` |
| fba52c3 | pydantic-ai: 5 emission sites |
| fec7c1d | ccb: 3 emission sites + hand-written TS enum |
| 0477f8f | certifier: 2-line consumer sync |
| 917bbdc | claude-code mapper: `logger.error` on UNSPECIFIED fallback |
| 5494af1 | L4: `_chunk_type_to_wire` single conversion + `response_text` accumulation fix + `tool_call_start` tolerance removed |
| b3fc066 | CLI: `_ALLOWED_CHUNK_TYPES` frozenset + `_render_chunk` unified render for `send`/`run` |
| 6fe7f60 | `tests/contract/cases/test_chunk_type_contract.py` (3 tests parametrized by `--runtime`) |
| 9c31623 | `.github/workflows/phase3-contract.yml` 7-runtime matrix + 8 Makefile targets |
| 0bf1cf8 | Checkpoint: state files + S4 runbook + deferred ledger |

### ADR amendment

`CHUNK_TYPE_WORKFLOW_CONTINUATION = 7` added to the enum mid-phase (D87 observability, grid harness.rs:557 → L4 event_interceptor.py:86). Wire value: `"workflow_continuation"`. Contract whitelist extended accordingly.

### Verification evidence

- `make v2-phase3-e2e` → **112 passed, 5 skipped** (17.50s)
- `make v2-phase3-e2e-rust` → aggregate_spill 15p + compaction_pipeline 15p + retry_graduated 4p (34/0)
- `make v2-phase3-contract-<rt>` for all 7:
  - grid / claude-code / nanobot / pydantic-ai / claw-code: PASS
  - goose / ccb: DEP-SKIP (local deps absent; CI runners have them)

### Lint note (F3/F4 governance gap)

`/adr:accept` bypassed manually — F1/F2/F5 PASS, F3 WARN (lint self-reference gap), F4 global pre-existing cross-ADR module conflicts unrelated to V2-021. Same pattern as ADR-V2-020 acceptance on 2026-04-18. Filed D151 to tighten lint's per-ADR F4 scoping.
