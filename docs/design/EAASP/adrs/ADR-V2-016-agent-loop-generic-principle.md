...
## Implementation Note (2026-05-20, Phase 5.3)

The implemented "D87 Fix 2" (`tool_choice=Required` forced continuation when
`required_tools` is unsatisfied) exceeded this ADR's original textual decision.
The mode gate `ExecutionMode{Conversational, LongWorkflow}` landed at commit
`f1999fb` (2026-05-16) to make that behavior opt-in.

**ADR-V2-026** (Accepted 2026-05-20) formalizes the mode gate retroactively and
supersedes this ADR. The capability matrix in §"E2E Verification Record" here
remains valid as the foundational principle — V2-026 layers the mode
discriminator on top:

- Default `execution_mode = Conversational` (REPL / TUI / `grid ask`)
- `grid-runtime` explicitly sets `LongWorkflow` for EAASP skill execution
- D87 Fix 2 forced continuation now only fires when `execution_mode == LongWorkflow`

The regression test `crates/grid-engine/tests/d87_multi_step_workflow_regression.rs`
remains the canonical lock for LongWorkflow behavior.
