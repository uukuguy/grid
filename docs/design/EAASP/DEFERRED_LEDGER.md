...
| **NEW-X2** | Sibling `kill_session` anti-pattern at `crates/grid-cli/src/commands/session.rs:99-103` (`delete_session`) and L157 (`export_session`) — same `anyhow!("Session not found: ...")` pattern as NEW-A3, still maps to ExitCode::General (1) instead of typed SessionNotFound (4) | 2026-05-22 Phase 5.5 Plan 01 RESEARCH §2.1 + Task 01.B1 scope limit per CONTEXT D-06 verbatim | 🟡 P3, defer to v3.2+ | **Deferred**: Plan 01 Task 01.B1 scope was kill_session only (per CONTEXT D-06 verbatim). Same `downcast_ref::<GridError>` pattern from main.rs:92-96 ports trivially to these sites (~5 LOC per site). Defer to v3.2+ typed-error rollup phase or opportunistic refactor when a future CLI hardening task touches commands/session.rs. |
| **NEW-X3** | `cargo build -p grid-cli --all-features` fails with 12 unrelated grid-engine errors (E0596 borrow-checker + E0412/E0425/E0433 in hooks module) — pre-existing as of 2026-05-22 | 2026-05-22 Phase 5.5 Plan 01 RESEARCH §Anti-Patterns R2 + Task 01.B3 plan note | 🟡 P3, defer to v3.2+ | **Deferred**: Out of Phase 5.5 scope (Plan 01 Task 01.B3 verified only default + studio features clean; all-features path exercises grid-engine code unrelated to grid-cli). 12 errors all in grid-engine hooks module — likely pre-existing tech debt from interim refactor. Defer to v3.2+ tech-debt batch sweep or grid-engine hooks hardening phase. |
| **NEW-X4** | `tests/contract/cases/test_chunk_type_contract.py::test_chunk_type_contract[<runtime>]` FAILS with `ScopeMismatch: You tried to access the function scoped fixture runtime_name with a session scoped request object` across 6 of 7 contract-matrix runtimes (claude-code / goose / nanobot / pydantic-ai / claw-code / ccb; hermes frozen-skip; grid still in_progress at observation time but expected same failure). Phase 3 Contract Matrix CI workflow has been RED since at least 2026-05-04 — pre-existing failure NOT introduced by Phase 5.5. | 2026-05-23 surfaced during Phase 5.5 post-push CI scan; root pre-existence confirmed via `gh run list --workflow="Phase 3 Contract Matrix"` showing FAIL on every push since 2026-05-04 | 🟠 P2, defer to v3.2+ early (blocks Phase 3 Contract Matrix CI for ALL runtimes — visible red signal) | **Deferred**: pytest fixture-scope mismatch — `runtime_name` is `@pytest.fixture(scope="function")` (or unscoped default) but a session-scoped fixture in `tests/contract/conftest.py` requests it. Likely root cause: `runtime_name` should be `scope="session"`, OR the requesting fixture should be `scope="function"`. Last contract test change was `3ba59a1 test(contract): parametrize chunk_type test for 7 runtimes (NEW-D2)` (Phase 5.1 2026-05-02) — likely introduced the scope conflict but wasn't caught locally because devs run targeted runtime tests, not the full matrix. NOT a Phase 5.5 regression (Phase 5.5 touched ZERO files under `tests/contract/`). Fix is small (~5 LOC scope-decorator change in `tests/contract/conftest.py` or `cases/test_chunk_type_contract.py`) but defer to v3.2+ since it requires running the full matrix locally to verify across all 7 runtimes. Local repro: `pytest tests/contract/cases/test_chunk_type_contract.py -k claude-code -v`. |

**合计新增 7 项 in this section**: 4 closed (NEW-E2, NEW-A3, NEW-L1, grid-cli mod output verify-only) + 3 P3/P2 deferred (NEW-X2 P3, NEW-X3 P3, NEW-X4 P2). The 4 closures land Phase 5.5 milestone close cascade; the 3 deferred rows feed v3.2+ inbox. NEW-X4 added post-push 2026-05-23 from CI scan (pre-existing failure, not Phase 5.5 regression).
