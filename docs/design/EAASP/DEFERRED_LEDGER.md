...
| ID | 标题 | 处理位置 | 影响 |
|----|------|----------|------|
| **D83** | grid-runtime ToolResult chunk 缺 `tool_name` | ✅ CLOSED 2026-04-14 Phase 2 S1.T4 @ bdc5b8b | runtime 侧工具识别（已修；衍生 D90 follow-up） |
| **D85** | `STOP` event `response_text` 空串 | ✅ CLOSED 2026-04-14 Phase 2 S1.T5 @ bdc5b8b | 上层 CLI 显示不出最终回答（已修 Rust+Python 双侧） |
| **D86** | claude-code-runtime SDK wrapper 丢 `ToolResultBlock` | ✅ CLOSED 2026-04-14 Phase 2 S1.T3 @ d0e6cb0 | POST_TOOL_USE hook 空链路（已修） |
| **D84** | CLI `session events --follow` SSE 未实现 | ✅ CLOSED 2026-04-15 Phase 2 S4.T2 @ bd55bc4 | CLI UX |
| **D89** | CLI `session close` 未实现 | ✅ CLOSED 2026-04-15 Phase 2 S4.T1 @ 28e6b21 | CLI UX |
...
| ID | 标题 | 建议挂靠 | 必须原因 |
|----|------|----------|----------|
| **D50** | `ScopedHookBody::Prompt` executor loop 未实装 | ✅ CLOSED 2026-06-10 Phase 8.5 Plan 03 @ 77566619 | SKILL 里 Prompt hook 类型功能上不存在 |
| **D53** | D49 helper 写了但 runtime 没调用 | ✅ CLOSED 2026-04-15 Phase 2 S3.T5 @ 7cb48eb | scoped-hook executor 真空 |
| **D51** | Hook stdin envelope schema 未 ADR 化 | ✅ CLOSED 2026-04-15 Phase 2 S3.T5 @ 7cb48eb | D50/D53 实施前必须先定义契约 |
| **D78** | Event payload embedding 向量索引 | ✅ CLOSED 2026-04-20 Phase 3 S2.T2 @ 4633c0bc | 与 semantic 检索共 HNSW 架构 |
...
| ID | 标题 | 证据 |
|----|------|------|
| **D87** | grid-engine agent loop 多步工作流早终止 | ✅ CLOSED 2026-04-14 Phase 2 S1.T1 @ bdc4fd5 |
...
| ID | 标题 | 引入 | 状态 | 证据 / 去向 |
|----|------|------|------|------|
| **D1** | grid-runtime harness 接入 `payload.policy_context` (P1) | ✅ CLOSED 2026-04-12 Phase 0.5 S4.T2 @ a6fad2b6 | ✅ closed | ADR-V2-004 S4.T2 4b-lite |
| **D2** | grid-runtime harness 接入 `payload.memory_refs` (P3) | ✅ CLOSED 2026-04-12 Phase 0.5 S4.T2 @ a6fad2b6 | ✅ closed | ADR-V2-004 `build_memory_preamble` |
| **D3** | harness 接入 `payload.user_preferences` (P5) + `trim_for_budget()` | phase0 S3.T3 | ✅ CLOSED 2026-06-07 Phase 7.0 v3.4 Plan 01 @ `65aa7bfe` | **Resolution**: Full D3 scope implemented (supersedes the v3.3 DEFERRED close @ `79d426c`). **(Sub-item A)** UserPreferences wired end-to-end: new `UserPreferences` type in `grid_engine::agent::loop_config` (user_id, language, timezone, llm_provider, llm_model — mirrors grid-runtime contract type minus HashMap field); added `user_preferences: Option<UserPreferences>` to `AgentLoopConfig`, `AgentExecutor`, `AgentRuntimeConfig`, and `AgentRuntime`; wired via `executor.set_user_preferences()` at `AgentRuntime::build_and_spawn_executor_filtered()` spawn path; consumed in the per-turn `AgentLoopConfig` struct-literal at `executor.rs`; `grid-runtime::GridHarness::initialize()` converts payload `UserPreferences` → engine `UserPreferences` and calls `runtime.set_user_preferences()` before `start_session_with_tool_filter()`. **(Sub-item B)** Budget-driven compaction: added `is_budget_tight()` pure helper to `harness.rs` (returns `remaining < MIN_TURN_BUDGET`); proactive compaction at `harness.rs` now gated on `task_budget_remaining < MIN_TURN_BUDGET` — when budget is sufficient (≥4096 tokens), proactive compaction is skipped with a `debug!` log; only proceeds when budget is genuinely tight. New tests: 15 D3 tests across `test_d3_user_preferences.rs` (5: type existence, field defaults, Clone, AgentLoopConfig field, struct-literal) + `test_d3_budget_driven_compact.rs` (10: budget arithmetic, can_continue, is_budget_tight, constants). `cargo test -p grid-engine -p grid-runtime` zero regressions (1598+ PASS). |
| **D4** | harness 接入 `payload.event_context` (P2) | ✅ CLOSED 2026-04-13 Phase 1 ADR-V2-002 | ✅ closed | Phase 1 ADR-V2-002 |
| **D5** | grpc_integration 测试迁移到 v2 telemetry envelope | phase0 S3.T3 | ✅ CLOSED 2026-06-02 Phase 7.1 Plan 01 @ `e2a3f91` | **Resolution**: Verify-not-rewrite per CONTEXT.md D-03. Ran `rg "telemetry_v1" -t rust crates/ tests/` → 0 hits. Ran `rg "TelemetryV1\|telemetry::v1\|telemetry_envelope_v1" -t rust crates/ tests/` → 1 hit on a documentation-only line at `crates/grid-runtime/tests/grpc_integration.rs:360` inside the `#[ignore = "TODO(s2.t1): rewrite for v2 terminate telemetry envelope"]` placeholder test (the `// v1 bundled a final_telemetry batch inside TerminateResponse;` comment is descriptive prose, not a code reference). Production code path uses `proto::TelemetryRequest` from the v2 envelope (verified at `grpc_integration.rs:234` `.emit_telemetry(proto::TelemetryRequest {...})`). `harness_payload_integration.rs:1-20` carries no v1 reference (plan-checker drift; the planned single-line comment update is a no-op). No production-code migration required. @ `e2a3f91`. |
| **D6** | certifier 补充 SessionPayload P1–P5 字段断言 | phase0 S3.T3 | ✅ CLOSED 2026-06-02 Phase 7.1 Plan 01 @ `5807fb7` | **Resolution**: Extended `tools/eaasp-certifier/src/blindbox.rs`'s `SessionPayload` builder pattern via a new `build_certifier_payload()` helper in `tools/eaasp-certifier/src/verifier.rs` that populates all 5 priority blocks (P1=PolicyContext, P2=EventContext, P3=memory_refs, P4=SkillInstructions, P5=UserPreferences) so the certifier exercises the full payload surface. Added `assert_session_payload_p1_p5(&proto::SessionPayload) -> Result<String, String>` in `verifier.rs` that strictly (per ADR-V2-028 lineage) FAILS the report if any P-block is missing or session_id+user_id are both empty. `verify_initialize` now builds the payload via `build_certifier_payload()`, invokes the assertion, and propagates a typed error on failure. `VerificationReport` gained a new `session_payload_schema_passed: bool` field (default-serialize true for backward compat). The `to_markdown` output in `tools/eaasp-certifier/src/report.rs` now includes a `\| SessionPayload schema \| P1✓ P2✓ P3✓ P4✓ P5✓ (CONTRACT-04 / D6) \|` row (or `FAIL (CONTRACT-04 / D6)` when the schema check failed). `cargo test -p eaasp-certifier --tests` 33/33 PASS including the new `markdown_report_renders_schema_fail` round-trip. @ `5807fb7`. |
| **D7** | EmitEvent 真实实现 | ✅ CLOSED 2026-04-11 Phase 1 ADR-V2-001 | ✅ closed | Phase 1 ADR-V2-001 |
| **D8** | `access_scope` 真实 RBAC 执行 | phase0 S3.T1 | ✅ CLOSED 2026-06-07 Phase 7.3 Plan 01 @ `6d3325ce` | **Resolution**: Implemented `require_access_scope()` FastAPI Depends() dependency extracting `X-Session-Scope` header. Missing header → 403. In `validate_session()`, hooks whose `access_scope` doesn't match the caller's scope (unless wildcard `*`) are skipped with `logger.warning("RBAC rejected", ...)`. Hooks without `access_scope` pass through (backward compat). 5 RBAC test cases verify missing header, mismatched scope, matching scope, wildcard bypass, and no-scope backward compat. All 39 L3 tests PASS. |
| **D9** | `skill_usage` 返回真实遥测 | phase0 S3.T1 | ✅ CLOSED 2026-06-07 Phase 7.3 Plan 01 @ `36eeb233` | **Resolution**: Added `AuditStore.skill_usage()` with L2 HTTP primary path (5s timeout, `{L2_MEMORY_ENGINE_URL}/v1/memory/{skill_id}/usage`) and L3 SQLite fallback (`SELECT COUNT/MIN/MAX FROM telemetry_events WHERE json_extract(payload_json, '$.skill_id') = ?`). New `GET /v1/telemetry/skill-usage?skill_id=X&since=ISO` endpoint RBAC-guarded via `require_access_scope()`. Response shape: `{"skill_id", "invocations", "first_seen", "last_seen", "source": "l2"|"l3_fallback"}`. 2 test cases: L3 fallback aggregation + 403 on missing scope header. |
| **D10** | S3.T1 MCP REST facade 升级为真 rmcp ServerHandler | ✅ CLOSED 2026-06-10 Phase 8.0 Plan 03 @ 28a9b15d (L3 dual-transport done; L2/L4 deferred to Phase 9.2) | [P3-async-when-touched] 🧹 tech-debt rationale: L2/L3/L4 MCP REST→rmcp ServerHandler unification; touch when MCP layer refactors | L2/L3/L4 统一切换契机 |
| **D11** | skill-registry `scope` 过滤在 `LIMIT` 之后 | ✅ CLOSED 2026-06-02 Phase 7.2 Plan 03 @ `382d7864` | **Resolution**: T01 @ `ee4d401a` added `access_scope TEXT` column to the `skills` table + idempotent `ALTER TABLE` migration + `CREATE INDEX IF NOT EXISTS idx_skills_access_scope` + a backfill UPDATE pass parsing v2 frontmatter for existing rows (rows without v2 frontmatter remain NULL). T02 @ `382d7864` rewrote `tools/eaasp-skill-registry/src/store.rs::search()` to apply the scope filter as `AND access_scope = ?` (or `IS NULL` for scope=None) INSIDE the main SELECT WHERE clause, BEFORE `LIMIT N`; deleted the post-query in-memory filter loop. `cargo test -p eaasp-skill-registry` PASS including the new scope-filter test (asserts `scope=X, limit=5` returns exactly the top-5 scope=X matches). Pre-LIMIT-correct behaviour: scope mismatch no longer wastes slots in the limit. @ `382d7864`. |
| **D12** | L2 memory-engine connection-per-call 延迟浪费 | phase0 S3.T2 | [P2-next-milestone] 🟢 **P2-active** rationale: L2 memory-engine MemoryStore singleton refactor; pairs with D94/D98 in L2 mega-sweep batch (see ROADMAP §v3.3 候选) | **S2.T1 前置**（hnswlib 接入必改） ✅ CLOSED 2026-06-02 Phase 7.2 Plan 01 @ 50a6996+ea0bbfc — verified substantively done in Phase 2.5 S2.T1+T6 (get_shared_connection + get_write_lock + per-path locks); Plan 01 T01 audit matrix confirmed zero residual aiosqlite.connect leaks in hot paths; Plan 01 T02 high-concurrency throughput test (10 coroutines × 10 ops, mixed read+write) completes in 851ms with 0 database-is-locked errors. AUDIT evidence in .planning/phases/07.2-l2-connection-pool-pipeline/AUDIT-T01.md. |
| **D13** | L2 `archive()` 创建 "archive of archive"，FTS 仍可搜 | ✅ CLOSED 2026-06-02 Phase 7.2 Plan 03 @ `079932cd` | **Resolution**: CONTEXT D-05 Option A. `MemoryFileStore.archive()` at `tools/eaasp-l2-memory-engine/src/eaasp_l2_memory_engine/files.py:284` now DELETEs every `memory_fts` row for the archived memory_id after the normal `write()` creates the new archived-version row (wrapped in `BEGIN IMMEDIATE` + `get_write_lock` per L2 connection-pool conventions). Both the archived version itself AND any historical confirmed/suggested versions disappear from FTS5 search. Defense-in-depth filter added at `index.py:~270` (FTS path) and `~L375` (HNSW union fetch): `AND mf.status != 'archived'`. L2's FTS5 schema at `db.py:102` is non-contentless so the DELETE syntax is correct. Test `tests/test_archive_isolation.py` 3/3 PASS in 0.16s — (a) post-archive search hides the memory, (b) fresh HybridIndex after restart still hides it, (c) archiving A doesn't affect unrelated B. @ `079932cd`. |
| **D14** | L2 `index._row_to_memory` 跨模块访问私有符号 | ✅ CLOSED 2026-06-10 Phase 8.4 Plan 01 @ 20d3f443 | [P3-async-when-touched] 🧹 tech-debt rationale: L2 cross-module private symbol access; mechanical refactor when L2 next touched | 重构为公共符号 |
| **D15** | L2 memory-engine 缺 `[tool.ruff]` / `[tool.mypy]` | ✅ CLOSED 2026-06-10 Phase 8.4 Plan 02 @ 8625f755 | [P3-async-when-touched] 🧹 tech-debt rationale: L2 lint config alignment; touch when L2 dev-config sweep | 统一 lint 配置 |

### D16–D26: Phase 0 S3.T3 (L3 governance) — 全部 tech-debt 或运维

| ID | 标题 | 状态 | 备注 |
|----|------|------|------|
| **D16** | L3 policy_engine.deploy() 在 commit 前读 `created_at` | ✅ CLOSED 2026-06-10 Phase 8.0 Plan 02 @ b4a2f517 | SQLite RETURNING 子句 |
| **D17** | L3 validate_session() `hook["hook_id"]` KeyError 风险 | ✅ CLOSED 2026-06-07 Phase 7.3 Plan 01 @ `4ad31691` | **Resolution**: Replaced `hook["hook_id"]` with `hook.get("hook_id")` + `HTTPException(422, "invalid_hook")` on None. Added test that verifies 422 on hook missing hook_id (via direct DB insert to bypass deploy-time Pydantic validation). |
| **D18** | L3 validate_session() 对 `session_id` path param 不校验 | ✅ CLOSED 2026-06-07 Phase 7.3 Plan 01 @ `c763b304` | **Resolution**: Added `Annotated[Path(min_length=1, pattern=r"^[a-zA-Z0-9_-]+$")]` to `session_id` param. 3 tests: empty→404, invalid chars (space)→422, valid→200. Used `Annotated` to avoid Python non-default-after-default error. |
| **D19** | L3 switch_mode() 接受任意 hook_id 静默创建 override | ✅ CLOSED 2026-06-10 Phase 8.0 Plan 02 @ 2a4f87f6 | warn 或 404 |
| **D20** | `_sanitize_errors()` 仅在 L3 定义，L2 也需要 | ✅ CLOSED 2026-06-10 Phase 8.0 Plan 01 @ 95d91963 | 抽到 `eaasp_common` |
| **D21** | L3 `managed_settings_versions` / `telemetry_events` 无保留策略 | 📦 long-term — L3 retention policy deferred to Phase 4+ ops-side rollout| 运维侧 TTL 策略 |
| **D22** | L3 无全局 FastAPI exception handler | ✅ CLOSED 2026-06-07 Phase 7.3 Plan 01 @ `68294bc8` | **Resolution**: Added `@app.exception_handler(ValidationError)` → 422 JSON with sanitized field errors + `@app.exception_handler(Exception)` → 500 JSON `{"error": "internal_error", "detail": ...}` (capped at 500 chars). Imported `JSONResponse` from starlette.responses. |
| **D23** | L3 无 loguru/logging 初始化 | ✅ CLOSED 2026-06-07 Phase 7.3 Plan 01 @ `c5a9c643` | **Resolution**: Added `loguru>=0.7` to pyproject.toml. Configured loguru in `create_app()` lifespan with stderr sink + ISO format. Strict-by-default `L3_LOG_LEVEL` validation at startup (per ADR-V2-028). Logger calls at key decision points: policy deploy (info), telemetry ingest (debug), session validate (debug), RBAC reject (warning). |
| **D24** | IDE Pyright missing-import 假阳性 | ✅ CLOSED 2026-06-13 Phase 8.6 Plan 01 @ 83dae165 | DevEx, pyrightconfig.json |
| **D25** | L3 无并发部署 E2E (HTTP 栈) | ✅ CLOSED 2026-06-10 Phase 8.0 Plan 02 @ 54616d22 | 运维侧 load test |
| **D26** | L3 tests 用 `time.sleep(1.1)` 防撞秒 | ✅ CLOSED 2026-06-07 Phase 7.3 Plan 01 @ `456e7284` | **Resolution**: Added `tiebreaker: int = Field(default=0, alias="_tiebreaker")` to `TelemetryEventIn`. Column added to schema + idempotent `ALTER TABLE` migration. `ORDER BY received_at DESC, tiebreaker DESC, event_id DESC`. Replaced all `asyncio.sleep(1.1)` in tests with explicit `_tiebreaker` values. `import asyncio` removed from test_audit.py. Tests complete in <0.1s (was ~2.2s). |

### D27–D45: Phase 0 S3.T4+ (L4 + CLI v2)

| ID | 标题 | 状态 | 备注 |
|----|------|------|------|
| **D28** | L4 无全局 exception handler (D22 复现) | ✅ CLOSED 2026-06-10 Phase 8.2 Plan 02 @ b5c545ff | 与 D22 合并 |
| **D29** | L4 `/v1/sessions/{id}/*` path param 未校验 | ✅ CLOSED 2026-06-10 Phase 8.2 Plan 02 @ b5c545ff | 与 D18 合并 |
| **D30** | L2/L3 `busy_timeout=5000` 未统一 | [P3-async-when-touched] 🧹 tech-debt rationale: L2/L3 busy_timeout unification; touch when eaasp_common.connect() refactors | `eaasp_common.connect()` ✅ CLOSED 2026-06-02 Phase 7.2 Plan 01 @ 341eec2 — BUSY_TIMEOUT_MS=5000 constant + L2_BUSY_TIMEOUT_MS env override (strict-by-default per ADR-V2-028) added to tools/eaasp-l2-memory-engine/src/eaasp_l2_memory_engine/db.py. Pragma now applied at both init_db and get_shared_connection so all L2 paths inherit. L4 (tools/eaasp-l4-orchestration/.../db.py:132) retains its literal 5000 with a docstring cross-reference; full eaasp_common.connect() helper deferred to v3.4+ per CONTEXT.md D-06 scope ceiling. |
| **D31** | L4 无 loguru 初始化 | ✅ CLOSED 2026-06-10 Phase 8.2 Plan 02 @ b5c545ff | 与 D23 合并 |
| **D32** | L4 无并发 `create_session` 压力测试 | 📦 long-term — L4 concurrency stress test deferred to Phase 4+ production load| 运维侧 load test |
| **D33** | L4 SESSION_CREATED 事件 payload 重复存储 | ✅ CLOSED 2026-06-10 Phase 8.3 Plan 01 @ `ea4e70af` | **Resolution**: SESSION_CREATED event now stores reference (`{session_id, reference_mode: true, payload_keys: [...]}`) instead of full SessionPayload. Full payload lives in sessions.payload_json only. Backward-compat via `reference_mode` flag. |
| **D34** | L4 无 Intent → skill_id NLU 解析 | ✅ CLOSED 2026-06-10 Phase 8.2 Plan 01 @ b9741ab3 | Phase 3+ NLU 或 L5 portal |
| **D36** | L4 event window 无 cursor (>10k 事件触发) | 📦 long-term — L4 event cursor deferred to Phase 3+ scale testing| Phase 3+ |
| **D37** | L4 `context_assembly` 硬编码 `allow_trim_p4=False` | ✅ CLOSED 2026-06-10 Phase 8.3 Plan 01 @ `b9a2b0b7` | **Resolution**: `allow_trim_p4` now reads from `L4_ALLOW_TRIM_P4` env var (default `false` for backward-compat). Configurable without code change. |
| **D38** | L4 `L2Client.search_memory` 未传 `user_id` | ✅ CLOSED 2026-06-10 Phase 8.2 Plan 01 @ ddc2cefc | 跨租户隔离，Phase 3 RBAC |
| **D39** | L4 `PolicyContext.policy_version` 用 `str(int)` 而非哈希 | ✅ CLOSED 2026-06-10 Phase 8.2 Plan 02 @ cf2694b5 | evidence chain 时顺带 |
| **D41** | eaasp-cli-v2 `session list` 无后端 endpoint | 📦 long-term — CLI session list multi-tenant sync deferred to Phase 4+| 多租户同步 |
| **D42** | cli-v2 test_client 未覆盖 5xx exit_code=4 | ✅ CLOSED 2026-06-10 Phase 8.3 Plan 02 @ cbf71505 | 补测 |
| **D43** | cli-v2 pyproject `respx>=0.21` 未使用 | ✅ CLOSED 2026-06-10 Phase 8.3 Plan 02 @ cbf71505 | 删除 dep |
| **D44** | cli-v2 `cmd_session.show` 硬编码 `limit=100` | ✅ CLOSED 2026-06-10 Phase 8.3 Plan 02 @ cbf71505 | S4 时顺带暴露 flag |
| **D45** | cli-v2 响应 shape 假设 → default exit 1 | ✅ CLOSED 2026-06-10 Phase 8.3 Plan 02 @ cbf71505 | response-shape guard |

### D46–D53: Phase 0 S4.T1 (Skill + Hook 扩展)

| ID | 标题 | 状态 | 备注 |
|----|------|------|------|
| **D46** | Skill `access_scope` 无 RBAC / 命名空间校验 | ✅ CLOSED 2026-06-07 Phase 7.3 Plan 01 @ `4d955506` | **Resolution**: Added namespace conflict check in `submit_draft()`: `SELECT id FROM skills WHERE access_scope = ? AND id != ? LIMIT 1`. Rejects with `Err("namespace_conflict: ...")` when a different skill already claims the scope. Same-skill re-registration allowed (UPDATE path). Wildcard `*` and None scope skip the check. 6 new tests in `tools/eaasp-skill-registry/tests/namespace_conflict.rs`. Updated `scope_filter.rs` to use unique scopes per skill (D46 constraint). |
| **D47** | mock-scada.py argparse stub | ✅ CLOSED 2026-04-12 Phase 0 S4.T2 | `tools/mock-scada/` (2026-04-12) |
| **D48** | `ScopedHookBody` 无 `matcher` / `tool_filter` 字段 | ✅ CLOSED 2026-06-10 Phase 8.5 Plan 02 @ e6b837a0 | hook schema v2.1 |
| **D49** | `${SKILL_DIR}` 变量替换 helper | ✅ CLOSED 2026-04-12 Phase 0 S4.T1 | `substitute_hook_vars` (2026-04-12) |
...
| **D52** | SKILL.md prose 与 L2 MCP tool schema 一致性 | ✅ CLOSED 2026-04-12 Phase 0 S4.T1 | 逐字对照验证 (2026-04-12) |
...
| ID | 标题 | 状态 | 备注 |
|----|------|------|------|
| **D54** | L4→L1 真 gRPC binding | ✅ CLOSED 2026-04-12 Phase 0.5 S1 | Phase 0.5 S1 |
| **D55** | proto3 submessage presence 应统一用 `HasField` | ✅ CLOSED 2026-06-02 Phase 7.1 Plan 01 @ `0842f60` | **Resolution**: Added 3 parity test modules pinning `HasField` / `Option::is_some` / `!== undefined` semantics for SessionPayload P1 / P4 / P5 submessages across Python, Rust, and TypeScript. Files: `tests/contract/contract_v1/test_proto3_hasfield_parity.py` (7 tests via `HasField`), `crates/grid-runtime/tests/proto3_hasfield_parity.rs` (6 tests via `Option::is_some/is_none`), `lang/ccb-runtime-ts/tests/proto3-hasfield-parity.test.ts` (3 tests via `!== undefined` checks — scoped to P4 only because the TS type surface at `lang/ccb-runtime-ts/src/proto/types.ts:34-44` currently types only SkillInstructions on SessionPayload). Mechanically prevents the truthy-fallback anti-pattern from re-emerging at zero ongoing maintenance cost — if a future refactor switches a presence check back to `if msg.field { ... }`, the parity tests fail. All 3 sides PASS: `cargo test -p grid-runtime --test proto3_hasfield_parity` 6/6, `pytest contract_v1/test_proto3_hasfield_parity.py` 7/7, `bun test tests/proto3-hasfield-parity.test.ts` 3/3. @ `0842f60`. |
| **D56** | `verify-v2-mvp.sh` 只清 SQLite | ✅ CLOSED 2026-06-13 Phase 8.6 Plan 01 @ 93ef61b8 | 持久化后端变化时 |
...
| **D59** | `Makefile::mcp-orch-start` 硬编码 `--port 8082` | ✅ CLOSED 2026-06-10 Phase 8.4 Plan 02 @ 3a54cb31 | 改为 18082 |
| **D60** | verify-v2-mvp assertion 11 hybrid search 降级 | ✅ CLOSED 2026-04-15 Phase 2 S2.T5 @ bad4269 | S2.T5 @ `bad4269` (升级为 `raise AssertionError`) |
| **D61** | `threshold-calibration-skill.md` fixture 硬编码 `version` | ✅ CLOSED 2026-06-10 Phase 8.3 Plan 02 @ c3c828a7 | 解析 submit 响应 |

### D62–D66: Phase 1 Plan (容器化 + MCP 池)

| ID | 标题 | 状态 | 备注 |
|----|------|------|------|
| **D65** | MCP server 多实例 / 连接池 | ✅ CLOSED 2026-06-10 Phase 8.5 Plan 01 @ a98a5653 | Phase 2 S2 或 Phase 3 顺带 |
...
| ID | 标题 | 状态 | 备注 |
|----|------|------|------|
| **D73** | Event Room 推迟 | 📦 long-term — Event Room deferred to Grid 独立产品 activation| Phase 4 |
| **D74** | EmitEvent gRPC 反向通道 (L1→L4 gRPC server) | ✅ CLOSED 2026-06-09 Phase 8.1 Plan 01 @ `30070df0` | **Resolution**: ADR-V2-033 Accepted — new `EventSink` gRPC service defined in `runtime.proto` with 2 RPCs (`SendEvent`, `StreamEvents`). L1→L4 reverse channel OPTIONAL per ADR-V2-001, reuses existing `EventStreamEntry` message. Implementation deferred per plan — ADR establishes contract for future L4 event streaming consumer. |
...
| **D90** | `ServerMessage::ToolResult` WS schema 缺 `tool_name` 字段（grid-server + grid-platform） | ✅ CLOSED 2026-06-09 Phase 8.1 Plan 03 @ e3fa59ce | 已解决 — WS 端 `tool_name` 在 grid-server map_event() 已完整传递（destructure + JSON payload），grid-platform 因 dormant 无需改动 |
| **D91** | HNSW 软删 tombstone rebuild 策略 | [P2-next-milestone] 🟡 P1-defer rationale: HNSW tombstone rebuild compaction; pairs with D94/D98/L2 in v3.3+ L2 mega-sweep batch | `mark_deleted` 标签单调累积；达到 N% 删除率后索引膨胀/搜索延迟劣化。需要触发阈值（建议 30%）+ 后台 compaction 任务。衍生自 S2.T1 → **Phase 2.5** ✅ CLOSED 2026-06-02 Phase 7.2 Plan 02 @ ca13d04+49f0cc4 — TOMBSTONE_REBUILD_THRESHOLD=0.30 + sync rebuild on next add() write-path piggyback (vector_index.py). `_deleted_count` persisted across save()/_try_load_sync() so debt survives process restarts. Test test_tombstone_rebuild.py covers under-threshold (29%) no-rebuild + at-threshold (30%) rebuild + recall preservation (100% on 16-survivor sample, score delta within ±0.05). Async rebuild deferred to v3.4+ per CONTEXT.md D-02. |
| **D92** | MockEmbedding 64-bit seed 碰撞审查 | ✅ CLOSED 2026-06-10 Phase 8.4 Plan 01 @ a0bd006b | SHA-256(text)[:8] 生日碰撞约 2^32。测试场景可接受；若被误用于 staging，两条不同文本可能撞同向量。加宽到完整 32-byte digest 或明确标注 "tests-only"。衍生自 S2.T1 → **Phase 3 GA 前** |
| **D93** | `embed_batch` 顺序实现 | [P2-next-milestone] 🟡 P1-defer rationale: embed_batch parallelization; perf-relevant for L2, pairs with D94/D98 in L2 mega-sweep | `OllamaEmbedding` / `MockEmbedding` 均 `for text in texts: await embed(text)` N 次。Ollama/TEI 均支持真正 batched POST。衍生自 S2.T1 → **S2.T4 或 Phase 2.5** hybrid-search perf pass ✅ CLOSED 2026-06-02 Phase 7.2 Plan 02 @ ec31ed4+26f2159 — embed_batch now uses asyncio.gather + per-provider asyncio.Semaphore (Mock=100, Ollama=5). L2_EMBED_BATCH_CONCURRENCY env override (strict-by-default per ADR-V2-028). Test test_embed_batch_concurrency.py confirms batch=10 wall-clock is 10.09% of sequential (52ms vs 511ms, delay=50ms) and 50-concurrent embed_batch entries respect concurrency=5 cap (max in-flight = 5). Native batched HTTP endpoints (Ollama / TEI) deferred to v3.4+ per CONTEXT.md D-03. |
| **D94** | MemoryStore 单例 + 共享连接重构（D12 收尾） | [P2-next-milestone] 🟡 P1-defer rationale: MemoryStore singleton refactor (D12 closeout); core to L2 v3.3+ mega-sweep, pairs with D12/D98 | S2.T1 仅完成 schema 迁移 + pack/unpack helper；`MemoryFileStore`/`AnchorStore`/`HybridIndex` 仍 per-call `connect()`。全 store 单例化需求较大，与 Phase 2.5 runtime ecosystem 工作合并推进 ✅ CLOSED 2026-06-02 Phase 7.2 Plan 01 @ 50a6996+ea0bbfc — D94 was D12's MemoryStore-singleton sibling; both close together. Phase 7.2 Plan 01 T01 audit confirmed all 3 stores (AnchorStore at anchors.py:50/91/99/108, MemoryFileStore at files.py:112/215/279, HybridIndex at index.py:295/384) consume get_shared_connection on every hot-path method. T02 throughput test proves the per-path singleton + write_lock combination withstands 10-coroutine concurrent load. |
| **D95** | FTS 命中的语义分数从 DB `embedding_vec` 回填 | ✅ CLOSED 2026-06-10 Phase 8.5 Plan 01 @ 6c2bc81a | S2.T2 union 阶段只对同时出现在 HNSW 结果中的 FTS 命中打 `semantic_score`；若 HNSW add 静默失败，FTS 命中永远 `sem_score=0`。可从 DB BLOB unpack + 与 query_vec 计算 cosine 回填。衍生自 S2.T2 → **S2.T4 或 Phase 2.5** |
| **D96** | 用户自定义 memory_id 含 `:v` 子串导致 HNSW key 解析丢失 | ✅ CLOSED 2026-06-10 Phase 8.4 Plan 01 @ 20d3f443 | HNSW key 格式 `{memory_id}:v{N}`，`split(":v")` 在 memory_id 含 `:v` 时产生 3 段被静默跳过。建议 (a) `MemoryFileIn.memory_id` 校验禁止 `:v`，或 (b) `rsplit(":v", 1)`。默认自动生成 `mem_{uuid}` 不受影响，仅用户传入自定 id 的边角场景。衍生自 S2.T2 reviewer M1 → **Phase 3 前** |
| **D97** | `weights=(0.0, 0.0)` 退化情形缺少构造期告警 | ✅ CLOSED 2026-06-10 Phase 8.4 Plan 01 @ 20d3f443 | 所有候选 `score==0`，插入序生效无信息。运维场景罕见，但建议 `HybridIndex.__init__` 下发 `logger.warning("Both weights zero; results will be unordered")`。衍生自 S2.T2 reviewer M2 → **Phase 2.5** |
| **D98** | `HybridIndex.search()` 每次重建 HNSWVectorIndex | [P2-next-milestone] 🟡 P1-defer rationale: HybridIndex per-search HNSW rebuild perf; pairs with D94 MemoryStore singleton in L2 mega-sweep | 每次 search 重新 `_try_load_sync()` 读磁盘 ~10ms；小索引可接受，QPS 上升后变成 perf 热点。承继 T1 同类问题。应随 D94 MemoryStore 单例化一起改为进程级缓存。衍生自 S2.T2 reviewer N3 → **Phase 2.5** ✅ CLOSED 2026-06-02 Phase 7.2 Plan 01 @ 7734a2b — verified already cached. HybridIndex._hnsw_cache (index.py:158) populated on first search and reused on every subsequent call (index.py:240-249). Plan 01 T03 added test_hybrid_index_cache.py non-regression test asserting cache pointer-identity across N searches; without this guard a future refactor dropping the cache would silently regress L2 hot-path by ~10ms per call. No source change required; CONTEXT.md D-07 path (3) applied. |
| **D99** | MCP dispatcher 参数类型强制转换抛原生 `ValueError`/`TypeError` 而非 `ToolError("invalid_arg")` | ✅ CLOSED 2026-06-10 Phase 8.4 Plan 01 @ 635617bf | `mcp_tools.py::_memory_list` / `_memory_search` 对 `limit`/`offset`/`top_k` 用 `int(args.get(...))` 直接转换；非法类型/字符串会抛原生异常，绕过 `_require` 的 `ToolError` 包装。预期下游 MCP SDK / REST body parsing 应先做 JSON-schema 校验，但 dispatcher 层应有兜底。不是 S2.T3 引入的回归，是承继既有模式。衍生自 S2.T3 reviewer → **Phase 2.5 dispatcher harden sweep** |
| **D100** | S2.T4 — `write()`/`confirm()`/`archive()` 构造 `MemoryFileOut` 时未 surface `embedding_model_id`/`embedding_dim` | ✅ CLOSED 2026-06-10 Phase 8.5 Plan 01 @ b91a4408 | 只有 `read_latest()` → `_row_to_memory` 返回完整字段，写路径 3 个 helper 对称性缺失。S2.T4 test 10 通过 docstring 记录，未断言。衍生自 S2.T4 reviewer → **Phase 2.5** |
| **D101** | FastAPI `HTTPException(detail=dict)` 嵌套 `'detail'` key 的契约 erratum | ✅ CLOSED 2026-06-10 Phase 8.4 Plan 01 @ 635617bf | S2.T4 REST 409/404 断言 `resp.json()['detail']['code']` 而非 `resp.json()['code']`；blueprint 描述用了扁平 shape。非 bug，是文档纠误。衍生自 S2.T4 reviewer → **下轮 blueprint 审校同步** |
...
| **D105** | S3.T1 — `HookPoint::ContextDegraded` 字符串别名保留 | ✅ CLOSED 2026-06-10 Phase 8.5 Plan 02 @ b861a7a0 | `runtime.rs:1899` string-key dispatcher 同时接受 `"ContextDegraded"` 和 `"PostCompact"`，保持既有 YAML/JSON hook 配置 backwards-compat。Phase 3 breaking release 时 deprecate 旧名。衍生自 S3.T1 coder → **Phase 3 breaking version** |
| **D106** | S3.T1 — `MAX_TURNS_FOR_BUDGET=50` 硬编码 | ✅ CLOSED 2026-06-09 Phase 8.1 Plan 02 @ `630572f0` | **Resolution**: Added `AgentLoopConfig.task_budget_multiplier: u32` field (default 50) following D102/D3 wiring precedent. Harness budget calculation now reads `config.task_budget_multiplier.max(1)` instead of the `MAX_TURNS_FOR_BUDGET` constant (which remains with `#[deprecated]` for backward compat). T-08.1-05 mitigated: `.max(1)` guard prevents zero-budget DOS. Tests updated to assert `AgentLoopConfig::default().task_budget_multiplier == 50`. Builder method `task_budget_multiplier()` exposes configurable override. |
| **D107** | S3.T2 — Stop hook 三-way empty-string 检查的共享 jq fragment | ✅ CLOSED 2026-06-10 Phase 8.5 Plan 03 @ 8d4d628c | `threshold-calibration/hooks/check_output_anchor.sh` 和 `skill-extraction/hooks/check_final_output.sh` 都用 `has(x) and (x != null) and (x != "")` 三-way check，S3.T2 reviewer C1 就是 `evidence_anchor_id` 漏掉 `!= ""` 分支导致空字符串被放行。Copy-paste 易漏，应抽成 skill-hook stdlib helper（例如 `${SKILL_DIR}/../../_lib/json_guards.sh` 或内嵌于 runtime hook executor）。衍生自 S3.T2 reviewer → **S3.T5 scoped-hook executor 带共享 lib 一起实现** |
| **D108** | S3.T2 — Hook script 自动化回归测试（bats/shellcheck） | ✅ CLOSED 2026-06-10 Phase 8.5 Plan 03 @ 6400ed8c | S3.T2 C1 靠 orchestrator 手动 4-case 回归才发现，没有持续 CI 保障。应加 `examples/skills/*/hooks/*.bats` 或 unified `scripts/test_hook_scripts.sh` 覆盖 allow/deny/edge-case envelope，集成到 `make verify`。衍生自 S3.T2 reviewer → **S3.T3 E2E 验证 或 Phase 2.5** |
...
| **D110** | S3.T2 — `dependencies` 字段 soft-intent vs runtime-required 语义不分 | ✅ CLOSED 2026-06-10 Phase 8.3 Plan 01 @ `c2b2ace0` | **Resolution**: `_normalize_skill_instructions()` now normalizes deps to `[{name, kind}]` dicts. `kind` accepts `"runtime"` or `"intent"`; flat string deps (backward-compat) default to `"runtime"`. Invalid kinds default to `"runtime"`. |
| **D124** | S4.T2 — L4 `/events/stream` 无 client-disconnect 结构化日志 | ✅ CLOSED 2026-04-15 Phase 2 S4.T2 | 4 结构化日志点：`sse_follow_start`（INFO, 入口）/ `sse_follow_session_gone`（INFO, mid-stream SessionNotFound）/ `sse_follow_idle_exit`（DEBUG, max_idle_polls 触发）/ `sse_follow_disconnect`（INFO, client 断开 → `asyncio.CancelledError` 捕获后 re-raise）。zero regressions (127/127 L4 tests PASS). ruff clean. |
| **D125** | S4.T2 — L4 events 流单次 poll 上限 500 events，burst 超限静默丢失 | ✅ CLOSED 2026-06-10 Phase 8.3 Plan 01 @ `b9a2b0b7` | **Resolution**: EventEngine now tracks a `_burst_counter` (per-second buckets) and `_check_burst_rate()` logs a WARNING when >500 events/sec are detected. Detection is observability-only; existing queue-full drop behavior unchanged. |
| **D126** | S4.T3 — `lang/claude-code-runtime-python/.venv` 缺失时 A8 fails late | ✅ CLOSED 2026-06-13 Phase 8.6 Plan 01 @ 93ef61b8 | `scripts/verify-v2-phase2.sh` 的 pre-flight `check_venv` 循环只覆盖 L2/L3/L4/cli-v2。若 fresh clone 未跑 `make claude-runtime-setup`，A8 要跑到一半才抛 AssertionError（message 指向 setup target 是清楚的，但前面 A1-A7 空跑了 ~30s）。应在 pre-flight 添加 WARNING（non-fatal，因为 skill extraction 是 fixture-replay-可选）。衍生自 S4.T3 reviewer M2 → **Phase 2.5 ergonomics** |
| **D127** | S4.T3 — `data/verify-v2-phase2-skill-registry/` 目录不被清理 | ✅ CLOSED 2026-06-13 Phase 8.6 Plan 01 @ 93ef61b8 | `verify-v2-phase2.sh` 的 wipe 块只清 `*.db`/`*.db-shm`/`*.db-wal` glob。但 skill-registry 用 `--data-dir` 挂到目录，里面有 `registry.db` + `skills/*` 子目录。重跑积累残留 manifest。目前无 Phase 2 assertion 查 registry（已核验），是 latent。MVP 的 `verify-v2-skill-registry/` 有同样 gap，继承行为不是新 regression。修法：在 wipe 块加 `rm -rf "$PROJECT_ROOT/data/verify-v2-phase2-skill-registry"`。衍生自 S4.T3 reviewer M3 → **Phase 2.5 when a Phase 2+ assertion starts reading registry state** |
| **D128** | S4.T3 — `@assertion` 装饰器 NOTE 在 PASS 之前打印（UX polish） | ✅ CLOSED 2026-06-13 Phase 8.6 Plan 01 @ 2861b1be | `@assertion` 装饰器先调用 wrapped function 再 print `PASS N. title`；function body 里的 `print()` NOTE（A5 graceful-degrade、A13 CLI 缺 .venv）会落在 PASS 行之前。阅读顺序是 `NOTE: ...\nPASS 5. ...`，略混乱但内容正确。可切到 post-hoc "notes" channel 或把 NOTE prefix 改为 `└─` 表示是 PASS 的细节。衍生自 S4.T3 reviewer N2 → **Phase 2.5 polish** |
| **D129** | S4.T3 — `verify-v2-phase2.sh` cleanup trap 在 pre-flight 失败时仍 sweep 外部端口 | ✅ CLOSED 2026-06-13 Phase 8.6 Plan 01 @ 93ef61b8 | `trap cleanup EXIT INT TERM` 无条件运行；`_kill_tree` 用 `$L2_PID` 等变量守护（pre-flight 失败时未设置 → no-op，正确），但 `_kill_port` 是按 PORT 号用 `lsof` sweep，不论这些端口上的进程是否由本脚本启动。实证：2026-04-15 运行 `make v2-phase2-e2e` 时 `make dev-eaasp` 正在另一终端跑，pre-flight 正确检测到 port 18085 被 PID 3398 占用并 exit 1，但随后 trap cleanup 无差别 sweep 了 6 个端口上的所有 listeners，杀掉了用户的 dev-eaasp session。修法：(a) 在 `_kill_port` 前检查对应的 `$*_PID` 是否非空（只 sweep "我们启动过的"），或 (b) pre-flight 失败时 `trap - EXIT` 提前 clear cleanup trap。生产 CI 无 pre-existing 服务不触发，是 local-dev 边角。衍生自 S4.T3 实跑观察 → **Phase 2.5 harness ergonomics** |
| **D130** | S4.T4 — Session-lifetime vs per-turn cancel token 双 token 不一致 | ✅ CLOSED 2026-06-09 Phase 8.1 Plan 02 @ `6e2b8cce` | **Resolution**: Added `LiveCancelToken` type in `cancellation_tree.rs` that wraps `Arc<AtomicBool>` shared flags from the `CancellationTokenTree`, checking live state on every `is_cancelled()` call instead of taking a snapshot. Replaced `TurnToken::to_cancellation_token()` (now deprecated) with `TurnToken::to_live_token()`. Changed `AgentLoopConfig.cancel_token` type from `CancellationToken` to `LiveCancelToken`; updated all construction sites (executor.rs, loop_.rs, subagent_runtime.rs, harness_integration.rs). Updated `execute_parallel()` to accept `&LiveCancelToken` (uses `.clone()` instead of `.child()`). Mid-turn session cancellation (e.g. `session kill` CLI) is now immediately visible to the harness loop via shared AtomicBool flags — no channel round-trip needed. 6 new D130 tests (mid-turn cancel visibility, turn cancel, default, clone sharing, session flag propagation, session isolation) all pass. T-08.1-07 mitigated: `SharedFlag` uses `AtomicBool` with Acquire/Release ordering; live-check eliminates time-of-check-to-time-of-use gap. |
...
| **D137** | Phase 2.5 S0.T4/T5 — Multi-turn observability + MCP bridge live + PRE_COMPACT 阈值触发 | ✅ CLOSED 2026-06-02 Phase 7.1 Plan 01 @ `03d420c` | **Resolution**: All 3 D137 sub-tasks landed across T01-T03 (10/10 xfails flipped): **(T01 @ `197b3ae`)** added `tests/contract/harness/multi_turn.py` multi-turn session replay framework (`MultiTurnFixture` + `TurnScript` + `drive_session`) + extended `mock_openai_server.py` to accept `{"kind": "text"}` script entries + flipped 2 CHUNK-channel D137 xfails in `test_event_type.py` (`test_chunk_event_is_emitted_for_assistant_text`, `test_tool_call_event_precedes_tool_result`) and 1 in `test_proto_shape.py::test_events_stream_emits_stop_at_turn_end` (assertions on `CHUNK_TYPE_TEXT_DELTA` / `TOOL_START` / `TOOL_RESULT` / `DONE` across multi-turn drives). **(T02 @ `a52b2c4`)** wired PRE_COMPACT emission via the EVENT channel (`HookEventType::PRE_COMPACT = 8` per `runtime.proto:246`, NOT a `ChunkType` variant) when `CompactionPipelineConfig.proactive_threshold_pct` is breached. Implementation path: `crates/grid-runtime/src/pre_compact_emitter.rs` HookHandler registered on `HookPoint::PreCompact` writes the `EventStreamEntry`-shaped JSON line to `${GRID_CONTRACT_PROBE_OUT}/events.jsonl` (the same probe-out sink already used by hook envelopes); observable from Python via the new `tests/contract/harness/event_log.py::fetch_captured_events` helper. The contract-test path uses the file sink rather than driving the `emit_event` gRPC RPC at `service.rs:597` because the probe harness consumes events out-of-band; the gRPC EmitEvent surface remains available for L4 callers that need streaming delivery. Added `GRID_COMPACTION_PROACTIVE_THRESHOLD_PCT` env override in `grid-runtime/src/main.rs`. Flipped the 2 remaining EVENT-channel D137 xfails in `test_event_type.py` (`test_unknown_event_type_not_emitted` + `test_pre_compact_event_emitted_over_threshold`). **The closed ChunkType enum was NOT modified** — anti-canaries enforced: `grep CHUNK_TYPE_PRE_COMPACT proto/eaasp/runtime/v2/common.proto` = 0; `git diff proto/eaasp/runtime/v2/` empty; `grep ChunkType::PreCompact crates/grid-runtime/src/harness.rs` = 0. **(T03 @ `03d420c`)** added `tests/contract/harness/test_mcp_server.py` thin stdio echo-tool MCP server + `mcp_subprocess.py` fixture (`McpSubprocess` + `mcp_subprocess_command` / `mcp_subprocess` pytest fixtures); flipped 5 D137 xfails in `test_mcp_bridge.py` (ConnectMCP stdio shape / round-trip ConnectMCP+DisconnectMCP / timeout-via-process-kill / disconnect-reconnect / unrecognised-transport error) plus 1 new orphan-detect smoke. 10/10 D137 xfails closed; no live LLM, no MCP server in the runtime path, no proto change. @ `03d420c`. |
| **D138** | Phase 2.5 S0.T4/T5 — skill-workflow enforcement 测试需可脚本化 deny-path mock LLM | ✅ CLOSED 2026-06-02 Phase 7.1 Plan 01 @ `e2460df` | **Resolution**: Extended `tests/contract/harness/mock_openai_server.py` + `mock_anthropic_server.py` with `X-Test-Scenario` header routing + `scenario_responses` parameter (T04). Added `tests/contract/harness/scenario_fixtures.py` with `OPENAI_DENY_SCENARIOS` / `ANTHROPIC_DENY_SCENARIOS` covering deny-non-required-tool / unknown-tool / multi-tool-permutation. Wired scenario propagation through the in-contract `UserMessage.metadata["x-test-scenario"]` map (no env-var shim — ADR-V2-028 strict-by-default respected): grid-runtime harness reads the metadata key, propagates it to a session-scoped value via `grid_engine::providers::test_scenario`, and `crates/grid-engine/src/providers/openai.rs` reads that value and sets `X-Test-Scenario` on the outbound `reqwest::RequestBuilder`. All 5 D138 xfails in `tests/contract/contract_v1/test_skill_workflow.py` flipped to live drive_session-based tests; deny-path emits CHUNK_TYPE_ERROR or refuses the offender's tool dispatch, no live LLM required. `cargo check -p grid-engine -p grid-runtime` PASS; `pytest test_harness_smoke.py` 9/9 PASS (scenario-routing infra); `pytest contract_v1/test_skill_workflow.py --runtime=grid` deferred to CI (Send-path hang unrelated to T05; same caveat applies to T01/T02 tests). @ `e2460df`. |
| **D139** | Phase 2.5 S0.T4 — 双 Terminate + 未知 session 语义未明 | ✅ CLOSED 2026-06-09 Phase 8.1 Plan 01 @ `6a329b17` | **Resolution**: ADR-V2-017 §2 revised — double-Terminate = idempotent NO-OP (second call returns empty OK, not FAILED_PRECONDITION); Send on unknown session = `NOT_FOUND` (not FAILED_PRECONDITION). Cross-runtime contract test (`tests/contract/test_terminate_semantics.py`, 169 LOC) enforces both semantics across 7 runtimes. ADR available at `docs/design/EAASP/adrs/ADR-V2-017-hermes-retirement.md` §2 addendum. |
...
| 🟡 **P1-defer** | 5 | D93, D102, D105, D109, D134, D136, D137 | 前置 frontend UI / Phase 3 breaking（D136/D137 via Phase 2.5 S0.T4-T6；D140 closed via Phase 3.6 T1；D90 closed 2026-06-09 @ `e3fa59ce`） |
...
| 2026-06-07 | D57 / D58 / D102 / D103 / D104 | Phase 7.0 Plan 01 early verification — all 5 D-rows re-verified ✅ CLOSED with commit hashes in LEDGER D-table. Regression suite passes: `cargo test -p grid-engine` (1598 PASS) + `cargo test -p grid-runtime` (PASS) + `cargo test -p grid-server --test compaction_yaml_wiring` (3/3 PASS) + `cargo test -p grid-engine find_tail_boundary_perf` (1/1 PASS). Pre-existing `ws_chunk_envelope` test failure (E0599 `test_inject_events`) is out-of-scope for Phase 7.0. |
| 2026-06-07 | D3 (ENGINE-02) | Phase 7.0 v3.4 Plan 01 — RE-OPENED from v3.3 DEFERRED then ✅ CLOSED with full implementation. Sub-item A: UserPreferences type + AgentLoopConfig field + wire path (grid-runtime initialize → AgentRuntime → AgentExecutor → AgentLoopConfig struct-literal). Sub-item B: budget-driven proactive compaction gate (`task_budget_remaining < MIN_TURN_BUDGET`). 15 new D3 tests + zero regressions across grid-engine (1598+ PASS) + grid-runtime (PASS). |
| 2026-06-07 | Phase 7.0 Plan 01 | CLOSE-OUT: all 6 ENGINE-01..06 REQ-IDs are ✅ CLOSED. D57/D58 @ `60f99d5`, D102 @ `3a23ecb`, D103 @ `3e0b055`, D104 @ `b32d985` (all re-verified). D3 @ `65aa7bfe` (newly implemented: user_preferences wiring + budget-driven compaction). D102 compaction_yaml_wiring 3/3 PASS. D103 find_tail_boundary_perf 1/1 PASS. Zero regressions across grid-engine / grid-runtime. Phase 7.0 complete. |
| 2026-06-08 | Phase 7.1 v3.4 Plan 01 | D5 ✅ / D6 ✅ / D55 ✅ / D137 ✅ / D138 ✅ (all re-verified CLOSED + all tests pass) | *Re-verification*: D5 (0 telemetry_v1 hits in crates/+tests/), D6 (eaasp-certifier 33/33 PASS), D55 (HasField parity 6+7+3=16/16 PASS across Rust/Python/TS), D137/D138 (grid-runtime zero regressions + harness smoke 9/9 PASS). All 5 commit hashes confirmed in git log (e2a3f91, 5807fb7, 0842f60, 03d420c, e2460df). Phase 7.1 v3.4 Plan 01 complete — pure verify-and-close, no implementation changes. |
| 2026-06-08 | Phase 7.2 v3.4 Plan 01 | D11 ✅ / D12 ✅ / D13 ✅ / D30 ✅ / D91 ✅ / D93 ✅ / D94 ✅ / D98 ✅ (all re-verified CLOSED + all L2 tests pass) | *Re-verification*: All 8 D-rows confirmed ✅ CLOSED with correct commit hashes in git log. Full L2 test suite 134 PASS (D91 tombstone rebuild 3/3, D93 embed_batch concurrency ~10x speedup 2/2, D98 HybridIndex cache pointer-identity 1/1, D13 archive isolation 3/3, D12/D94 throughput 0 db-locked 1/1, D11 scope filter 2/2, D30 BUSY_TIMEOUT_MS=5000 confirmed). 1 pre-existing V2 frontmatter failure (eaasp-skill-registry) unrelated to D11. Phase 7.2 v3.4 Plan 01 complete — pure verify-and-close, no implementation changes. |
| 2026-06-10 | D33, D37, D110, D125 | Phase 8.3 Plan 01 — ✅ CLOSED | **L4 P3 hardening**: D33 (SESSION_CREATED reference-mode @ `ea4e70af`), D37 (allow_trim_p4 env var @ `b9a2b0b7`), D110 (dependencies kind field @ `c2b2ace0`), D125 (event burst detection @ `b9a2b0b7`). All 4 implemented with tests + passed 158/160 L4 test suite (2 pre-existing failures). |
