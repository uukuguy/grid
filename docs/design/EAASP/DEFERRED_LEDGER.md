...
| **D5** | grpc_integration 测试迁移到 v2 telemetry envelope | phase0 S3.T3 | [P3-async-when-touched] 🤔 revisit-after-S2 rationale: v2 telemetry envelope migration awaits EmitTelemetry Terminate semantics — touch with L4 telemetry pass | EmitTelemetry Terminate 语义需先定 |
...
| **D57** | `harness_payload_integration.rs` 复制 `build_memory_preamble` | [P3-async-when-touched] 🧹 tech-debt rationale: harness_payload_integration.rs build_memory_preamble dedup; touch when harness next refactors | pub fn 升级 |
| **D58** | `test_initialize_injects_memory_refs_preamble` 未走 Send 全路径 | [P3-async-when-touched] 🧹 tech-debt rationale: test SdkWrapper replacement for full Send path; touch when integration tests next refactor | SdkWrapper 替身 |
...
| **D102** | S3.T1 — `AgentLoopConfig.compaction` 字段未接 YAML 配置层 | [P2-next-milestone] 🟡 P1-defer rationale: AgentLoopConfig.compaction YAML binding; small config wire-up in grid-server, batchable with config-hardening | 字段已加 + builder API 可用，但 `agent.yaml` → `AgentLoopConfigBuilder::compaction(...)` 的读取路径未接。下游 caller 通过 builder 传入；YAML 路径延后。衍生自 S3.T1 coder → **Phase 2.5 config harden sweep** (`crates/grid-server/src/api/agents.rs` + engine agent factory) |
| **D103** | S3.T1 — `find_tail_boundary()` O(N²) 重估风险 | [P3-async-when-touched] 🔵 P3-defer rationale: find_tail_boundary O(N²) risk; perf corner, deferred to Phase 3 perf pass | 当前实现每消息调用 `estimate_messages_tokens`，总复杂度 O(sum of msg sizes) 而非真 O(N²)。仅在极长 session + 大 `tail_protect_tokens` 时才成为热点。衍生自 S3.T1 coder → **Phase 3 perf pass** |
| **D104** | S3.T1 — 反应式 guard 在 harness 而非 pipeline | [P3-async-when-touched] 🔵 P3-defer rationale: Reactive guard pipeline placement; ADR-V2-018 §D5 compliant currently, defer until pipeline re-architect | `attempted_reactive_compact` 是 harness 循环本地状态，pipeline 层不 enforce。若未来其他 caller 绕过 harness 直接调 pipeline，同 turn double-compact 无锁。ADR-V2-018 §D5 明确 per-turn 语义，当前实现符合。衍生自 S3.T1 coder → **Phase 3 re-architect if needed** |
