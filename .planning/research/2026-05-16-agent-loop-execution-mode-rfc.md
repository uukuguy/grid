# RFC: Agent Loop Execution Mode — REPL vs Long-Workflow

**Status:** Draft (for Phase 5.3 plan-phase intake)
**Date:** 2026-05-16
**Author:** observed during deepseek-chat TUI shakedown session
**Supersedes precedent:** ADR-V2-016 (the *intended* scope, not the *implemented* code)
**Related ADRs:** ADR-V2-016 (Accepted 2026-04-14), ADR-V2-024 (双轴 model)
...
本 RFC 提议为 `AgentLoopConfig` 加 `execution_mode: ExecutionMode { Conversational, LongWorkflow }` 显式标识,把 ADR-V2-016 的"漂移"在新 ADR 里 retroactively 合法化,并按调用方区分默认值。**不改变 EAASP 当前已通过的 Phase 2/2.5/3 E2E 行为**,只是把"REPL 误命中长工作流逻辑"这条路径堵上。
...
- ❌ **不改 ADR-V2-016** — 它已 Accepted,改文字相当于篡改历史。新发 ADR-V2-026 supersede + 注脚说明 V2-016 实际授权范围
...
2. `harness.rs:1452`:在 D87 Fix 2 触发条件最前面加 `execution_mode == LongWorkflow &&`
...
7. **新测试**:`test_conversational_mode_no_forced_continuation` — TUI 场景的 regression test

### 4.2 ADR 层

发 ADR-V2-026 "Agent Loop Execution Mode (REPL vs Long-Workflow)":
- §Context:引用本 RFC + AGENT_LOOP_ROOT_CAUSE_ANALYSIS.md
- §Decision:`ExecutionMode` 枚举 + 调用方默认值表
- §Supersedes:Acknowledges ADR-V2-016 实际实现 (D87 Fix 2) 超出原 ADR 文字范围,本 ADR 是 retroactive 合法化 + 加 gate
- §Affected Modules:`loop_config.rs`、`harness.rs`、所有 ProviderConfig 构造点、3 个 runtime 入口

### 4.3 测试矩阵

至少这三个场景跑通才能 close 本 RFC:

| 场景 | 模型 | 预期 |
|---|---|---|
| TUI × deepseek-chat × web_search × Conversational | deepseek-chat | 调一次 web_search 后自然语言总结 → 退出,等下一个 user input |
| TUI × claude-sonnet × 多 tool 任务 × Conversational | claude-sonnet | 连续调多个 tool (Claude 主动性),完成后退出 |
| EAASP skill × grid-runtime × LongWorkflow | OpenAI/GPT-4o | threshold-calibration ≥4 PRE_TOOL_USE 完整跑通 (D87 regression PASS) |
...
- ✅ 显式 mode 比启发式 (`required_tools.is_some()`) 更可推理,符合 CLAUDE.md §"Karpathy guidelines" 的"surface assumptions"
- ✅ 为未来可能的第三种模式 (例如 batch/cron) 留口子
...
- 🚨 **migration regression**:grid-runtime 漏改 → EAASP E2E 回归。**缓解**:Phase 5.3 plan 把 grid-runtime 的 ExecutionMode 设置作为 must-have task + D87 regression test 双重锁定
- 🚨 **第三种调用方未覆盖**:`grid-platform` 现 dormant,激活时可能有不同需求。**缓解**:Phase 5.3 暂时不解决,等 Leg B 激活按 ADR-V2-024 双轴 framework 决策
- 🚨 **ExecutionMode 后续变成"什么都丢进去"的杂物袋**(autonomous mode、dual-mode、planning mode...):**缓解**:本 RFC 限定枚举只能扩展为新 variant,不允许加正交 bool flag

---

## 6. Alternatives Considered

### 6.1 Option A: 仅靠 `required_tools.is_some()` gate

**否决理由**:这是 D87 Fix 2 现状,已经被实证不够 — EAASP skill 漏写 required_tools 时静默回退,grid-cli TUI 永远不会有 required_tools 但用户偶尔想跑长任务也无路可走。

### 6.2 Option B: harness 启发式检测 ("调用方是 TUI 还是 gRPC?")

**否决理由**:harness 不该感知调用栈 → 违反分层。CLAUDE.md §6 "Single responsibility per function" 也排斥。

### 6.3 Option C: 完全删除 D87 Fix 2,回到 ADR-V2-016 原始一行版本

**否决理由**:EAASP Phase 2.5/3 整个验收建立在 D87 Fix 2 之上,删除等于退回 ADR-V2-016 失败状态,违反 ADR-V2-024 双轴 "engine 接入面优先" 的工时投入决策。

### 6.4 Option D: 上层在 prompt 里塞 "不要中途问用户" 指令

**否决理由**:ADR-V2-016 第 99-103 行明确否决了 system prompt 修正 — "prompt 不该承担 loop 控制责任"。本 RFC 同意。

---

## 7. 验收

Phase 5.3 plan-phase 时本 RFC 应被作为输入。Plan 必须包含:

1. `ExecutionMode` 枚举 + 字段实现 (single commit)
2. 3 个调用方 wire-up (single commit, 同 phase)
3. D87 regression test fixture 更新 (单元测试维度)
4. 新增 conversational regression test:验证 deepseek-chat + web_search 不再被强制续航
5. ADR-V2-026 起草、accept (走 `/adr:accept` lint)
6. ADR-V2-016 文末加 "**Implementation note (2026-05-16):** the implemented Fix 2 exceeded this ADR's textual decision. See ADR-V2-026 for retroactive scope alignment." 注脚 — 不改 status,留历史

---

## 8. 关联

- `crates/grid-engine/src/agent/harness.rs:70-90`(常量+注释)、`L474-477`(counter)、`L1423-1560`(D87 Fix 2 主逻辑)
- `crates/grid-engine/src/agent/loop_config.rs`(目标:加 ExecutionMode 字段)
- `crates/grid-engine/src/agent/continuation.rs`(独立 module,**不在本 RFC 范围**)
- `crates/grid-engine/tests/d87_multi_step_workflow_regression.rs`(锁定 LongWorkflow 行为)
- `docs/design/EAASP/AGENT_LOOP_ROOT_CAUSE_ANALYSIS.md`(D87 原始根因)
- `docs/design/EAASP/adrs/ADR-V2-016-agent-loop-generic-principle.md`(被 supersede 的目标)
- 本 RFC 文件本身将随 Phase 5.3 plan-phase 进入正式工作流
